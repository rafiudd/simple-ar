...
export default function OrderPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // mapping item cart -> payload sesuai API
      const payload = {
      ...
      // response: { statusCode, message, data, total_price }
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', maxWidth: 520, margin: '0 auto' }}>
      <button
        onClick={() => navigate('/')}
        style={{ background: 'none', border: 'none', color: '#007bff', cursor: 'pointer', marginBottom: 20 }}
      >
        ← Kembali ke Menu
      </button>
